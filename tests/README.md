...
```bash
git clone https://github.com/devSviat/OrdersExport-OkayCMS.git
```
...
```
{OkayCMS_root}/
├── Okay/Modules/Sviat/OrdersExport/     ← модуль
└── tests/Modules/Sviat/OrdersExport/     ← його тести
```
...
```bash
cd {OkayCMS_root}
mkdir -p tests/Modules/Sviat/OrdersExport
cp -r ~/OrdersExport-OkayCMS/tests/. tests/Modules/Sviat/OrdersExport/
```
...
```bash
cd {OkayCMS_root}
vendor/bin/phpunit tests/Modules/Sviat/OrdersExport
```
...
2. Неймспейс — `Modules\Sviat\OrdersExport`.
...
use Okay\Modules\Sviat\OrdersExport\Services\SomeService;
